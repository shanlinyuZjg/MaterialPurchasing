...
{
    public partial class SpecialItem : Office2007Form
    {
        public SpecialItem()
        {
            InitializeComponent();
        ...
        {
            string sqlSelect = @"Select ItemNumber AS 物料代码,ItemDescription AS 描述  From PurchaseDepartmentStockSpecialItem";
        ...
        private void tbItemNumber_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13)
            {
                ...
                {
                    string sqlCheckExist = @"Select Count(Id) From PurchaseDepartmentStockSpecialItem Where ItemNumber = '" + tbItemNumber.Text + "'";
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbItemDescription.Text))
            {
                ...
                if (!SQLHelper.Exist(GlobalSpace.FSDBConnstr, sqlCheckExist))
                {
                    string sqlInsert = @"Insert Into PurchaseDepartmentStockSpecialItem (ItemNumber,ItemDescription) Values ('" + tbItemNumber.Text + "','" + tbItemDescription.Text + "')";
